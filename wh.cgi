...
if (length($cgi->param('words')) > 0) {
    ...
    chop($str);
    chop($str);
    $sql = "CREATE TEMP VIEW tmpview AS SELECT c1,c2,c3,c4,c5,c6,c7,c8,c9,c10 FROM heritage_corpus WHERE $str ; CREATE TEMP VIEW sumview AS SELECT sum(c1) AS c1, sum(c2) AS c2, sum(c3) AS c3, sum(c4) AS c4,sum(c5) AS c5,sum(c6) AS c6,sum(c7) AS c7,sum(c8) AS c8,sum(c9) AS c9,sum(c10) AS c10 FROM tmpview; SELECT world_heritage.name, world_heritage.country, world_heritage.area, world_heritage.longitude, world_heritage.latitude, inner_product(heritage.c1, heritage.c2, heritage.c3, heritage.c4, heritage.c5, heritage.c6, heritage.c7, heritage.c8, heritage.c9, heritage.c10, sumview.c1, sumview.c2, sumview.c3, sumview.c4, sumview.c5, sumview.c6, sumview.c7,sumview.c8, sumview.c9, sumview.c10) AS score FROM world_heritage, sumview ORDER BY score DESC;";
} else {
