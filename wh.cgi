...
if (length($cgi->param('words')) > 0) {
    ...
    chop($str);
    chop($str);
    $sql = "CREATE TEMP VIEW tmpview AS SELECT c1,c2,c3,c4,c5,c6,c7,c8,c9,c10 FROM heritage_corpus WHERE word='cultural' ; CREATE TEMP VIEW sumview AS SELECT sum(c1) AS c1, sum(c2) AS c2, sum(c3) AS c3, sum(c4) AS c4,sum(c5) AS c5,sum(c6) AS c6,sum(c7) AS c7,sum(c8) AS c8,sum(c9) AS c9,sum(c10) AS c10 FROM tmpview; SELECT world_heritage.name, world_heritage.country, world_heritage.area, world_heritage.longitude, world_heritage.latitude, inner_product(world_heritage.c1, world_heritage.c2, world_heritage.c3, world_heritage.c4, world_heritage.c5, world_heritage.c6, world_heritage.c7, world_heritage.c8, world_heritage.c9, world_heritage.c10, sumview.c1, sumview.c2, sumview.c3, sumview.c4, sumview.c5, sumview.c6, sumview.c7,sumview.c8, sumview.c9, sumview.c10) AS score FROM world_heritage, sumview ORDER BY score DESC;";
} else {
